...
import {exchangePagesToken,loadPagesSource} from '@/lib/pages-runtime';
...
export default function Home({pagesMode=false}:{pagesMode?:boolean}={}){
 const [mode,setMode]=useState(pagesMode?'text':'url'),[input,setInput]=useState(''),[key,setKey]=useState(''),[model,setModel]=useState(''),[note,setNote]=useState(''),[pieces,setPieces]=useState<Piece[]>([]),[showSource,setShowSource]=useState(true),[dark,setDark]=useState(false),[fontSize,setFontSize]=useState(18),[busy,setBusy]=useState(false),[status,setStatus]=useState(''),[error,setError]=useState(false),[demo,setDemo]=useState(false),[copied,setCopied]=useState(false);
 ...
 async function translate(){
  ...
  try{
   ...
   const refreshVertexToken=async()=>{
    ...
    if(pagesMode){data=await exchangePagesToken(assertion,ctrl.signal)}else{
     ...
     const result=await response.json();if(!response.ok)throw new Error(result.error||'Vertex 인증에 실패했사와요.');data=result;
    }
   ...
   if(mode==='url'&&pagesMode){source=await loadPagesSource(source,ctrl.signal)}
   else if(mode==='url'){
